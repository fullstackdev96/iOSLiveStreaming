...
- (void)setupEncoders
{
    ...
    self.videoWidth = 640;
    int audioBitrate = 96000; // 64 * 1024; // 64 Kbps
    int videoBitrate = 1000000; //1024 * 1024; // 3 Mbps
#if HardwareEncode
...
#else
    self.ffmpegVideoEncoder = [[FFmpegVideoEncoder alloc] initWithBitrate:videoBitrate width:self.videoWidth height:self.videoHeight directory:self.folderName url:self.sUrl];
    self.ffmpegVideoEncoder.delegate = self;
#endif
...
- (void)setupVideoCapture
{
    ...
    NSDictionary *captureSettings = [[NSDictionary alloc] initWithObjectsAndKeys:
...
- (void)encoder:(KFEncoder *)encoder encodedFrame:(KFFrame *)frame
{
    ...
    } else if (encoder == self.ffmpegVideoEncoder){
        
        KFVideoFrame *videoFrame = (KFVideoFrame*)frame;
        CMTime scaledTime = CMTimeSubtract(videoFrame.pts, self.originalSample);
        [self.hlsWriter processEncodedData:videoFrame.data presentationTimestamp:scaledTime streamIndex:0 isKeyFrame:NO];
...
//        [self.hlsWriter processEncodedData:frame.data presentationTimestamp:scaledTime streamIndex:1 isKeyFrame:NO];
        ...
        [self.ffmpegVideoEncoder encodeAudioData:frame.data presentationTimestamp:scaledTime];
    }
}
...
#else
        [self.ffmpegVideoEncoder encodeSampleBuffer:sampleBuffer];
...
//    [self.hlsWriter prepareForWriting:&error];
    ...
    [self.ffmpegVideoEncoder startTimer];
...
{
    self.isRecording = NO;
    [self.ffmpegVideoEncoder finishTimer];
