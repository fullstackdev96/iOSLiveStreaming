...
- (void)addVideoStreamWithWidth:(int)width height:(int)height
{
    ...
    _videoStream.stream->codec->pix_fmt = PIX_FMT_YUV420P;
    _videoStream.stream->codec->gop_size = 10;
    _videoStream.stream->codec->qmin = 10;
    _videoStream.stream->codec->qmax = 51;
...
- (void)processEncodedData:(NSData *)data presentationTimestamp:(CMTime)pts streamIndex:(NSUInteger)streamIndex isKeyFrame:(BOOL)isKeyFrame
{
    ...
    dispatch_async(_conversionQueue, ^{
      ...
      // This lets the muxer know about H264 keyframes
        ...
        if (streamIndex == 1)
        {
            av_bitstream_filter_filter(aacbsfc, _outputFile.formatContext->streams[_packet->stream_index]->codec, NULL, &_packet->data, &_packet->size, _packet->data, _packet->size, 0);

        }
        if (streamIndex == 0)
	            NSLog(@"video packet");
//        NSLog(@"Streaming Index : %d, size = %d, PTS=%llu", streamIndex, data.length, originalPTS);
        ...
        NSLog(@"Test 1 Streaming Index : %d, size = %d, PTS=%llu", streamIndex, data.length, scaledPTS);
